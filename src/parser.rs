...
/// - `8:00-08:30`
/// - `8-12`
pub(crate) fn parse_interval_arg(input: &str) -> Option<Duration> {
...
pub fn parse_arg(arg: &str) -> Result<Duration, TimecalcError> {
    if let Some(duration) = parse_duration_arg(arg) {
        return Ok(duration);
    }

    if let Some(interval) = parse_interval_arg(arg) {
        return Ok(interval);
    }
...
mod tests {
    ...
    fn test_parse_interval() -> Result<()> {
        assert_eq!(
            parse_interval_arg("08:00-12:00").unwrap(),
            Duration::hours(4)
        );
        assert_eq!(
            parse_interval_arg("08:00-08:30").unwrap(),
            Duration::minutes(30)
        );
        assert_eq!(parse_interval_arg("8-12").unwrap(), Duration::hours(4));
        assert_eq!(parse_interval_arg("8:00-09").unwrap(), Duration::hours(1));
        assert_eq!(parse_interval_arg("8-9").unwrap(), Duration::hours(1));
        Ok(())
    }
